...
export class SearchApi {
     ...
     */
    public searchUsingPOST(type: string, query?: any, size?: number, page?: number, order?: string, extraHttpRequestParams?: any): Observable<models.PageMapstringobject> {
        return this.searchUsingPOSTWithHttpInfo(type, query, size, page, order, extraHttpRequestParams)
            .map((response: Response) => {
                if (response.status === 204) {
                    return undefined;
                } else {
     ...
     * @param order a comma separated list of sorting requirements in priority order, each entry matching PROPERTY_NAME:[ASC|DESC]
     */
    public searchUsingPOSTWithHttpInfo(type: string, query?: any, size?: number, page?: number, order?: string, extraHttpRequestParams?: any): Observable<Response> {
        const path = this.basePath + `/search/index/${type}`;
